...
namespace GameLib
{
    public class Input
    {
        ...
        public static bool Q, W, E, R, T, Y, U, I, O, P, A, S, D, F, G, H, J, K, L, Z, X, C, V, B, N, M,
                           UP, DOWN, LEFT, RIGHT, ONE, TWO, THREE, FOUR, FIVE, SIX, SEVEN, EIGHT, NINE, ZERO,
                           SPACE, ESC, SHIFT, CONTROL, ALT;

        internal static void GetKeyUp()
        {
            if (Game.KeyPresses == Keys.Alt) { ALT = false; }
            if (Game.KeyPresses == Keys.Control) { CONTROL = false; }
            if (Game.KeyPresses == Keys.Shift) { SHIFT = false; }
            if (Game.KeyPresses == Keys.Escape) { ESC = false; }
            if (Game.KeyPresses == Keys.Space) { SPACE = false; }
        ...
        {
            if (Game.KeyPresses == Keys.Alt) { ALT = true; }
            if (Game.KeyPresses == Keys.Control) { CONTROL = true; }
            if (Game.KeyPresses == Keys.Shift) { SHIFT = true; }
            if (Game.KeyPresses == Keys.Escape) { ESC = true; }
            if (Game.KeyPresses == Keys.Space) { SPACE = true; }
